...
const ISSUE_COLUMNS = 'id, unit_id, unit_stage_id, title, description, priority, assigned_to, status, due_date, created_at, updated_at'
...
function normalizeIssuePatch(payload, { requireTitle = false } = {}) {
  ...
  if (requireTitle && !title) {
    throw new Error('Título da issue é obrigatório.')
  }
  ...
  if (requireTitle || Object.prototype.hasOwnProperty.call(payload || {}, 'due_date')) {
    const dueDate = safeStr(payload?.due_date).trim()
