...
const ISSUE_COLUMNS =
  'id, tenant_id, project_id, unit_id, unit_stage_id, title, description, priority, assigned_to, status, created_at, updated_at'
...
export async function createIssue(payload) {
  ...
  const tenantId = requireId(payload?.tenant_id, 'tenant_id')
  const projectId = requireId(payload?.project_id, 'project_id')
  const unitId = requireId(payload?.unit_id, 'unit_id')
  const unitStageId = requireId(payload?.unit_stage_id, 'unit_stage_id')
